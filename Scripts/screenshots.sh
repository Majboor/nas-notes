...
for p in home note clips search fav toast; do "$B/snap" "$p" "$IMG/$p.png"; done

# optional GIF
if command -v ffmpeg >/dev/null && command -v magick >/dev/null; then
  for f in home note clips search toast; do magick "$IMG/$f.png" -background '#232326' -gravity north -extent 1200x1360 "$IMG/pad_$f.png"; done
  L="$B/list.txt"
  { for f in home note clips search toast; do echo "file '$IMG/pad_$f.png'"; echo "duration 1.6"; done; echo "file '$IMG/pad_toast.png'"; } > "$L"
